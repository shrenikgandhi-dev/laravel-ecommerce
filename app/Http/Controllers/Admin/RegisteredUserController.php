...
class RegisteredUserController extends Controller
{
    ...
    {
        return Inertia::render(config('constants.ADMIN_DIR_UC') . '/Register');
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            'uname' => [
                ...
                'string',
                'max:255',
                Rule::unique(Admin::class)->where(fn (Builder $query) =>
        ...
        Auth::guard(config('constants.ADMIN_DIR'))->login($user);

        return redirect(route(name: config('constants.ADMIN_DIR') . '.dashboard', absolute: false));
    }
}
